...
func BenchmarkGRPCSrv(b *testing.B) {
	...
	flatbuffer.CounterStart(builder)
	flatbuffer.CounterAddAnnotation(builder, anno)
	flatbuffer.CounterAddId(builder, id)
	flatbuffer.CounterAddClientTimeNanos(builder, 1234566)
	flatbuffer.CounterAddValue(builder, 1337)
	counter := flatbuffer.CounterEnd(builder)
...
func makeStreamMock() *StreamMock {
	return &StreamMock{
		...
		sentFromServer: make(chan *flatbuffers.Builder, 10),
	}
}
...
type StreamMock struct {
	grpc.ServerStream
	ctx            context.Context
	sentFromServer chan *flatbuffers.Builder
}
...
func (m *StreamMock) Send(resp *flatbuffers.Builder) error {
	return nil
}

func (m *StreamMock) Recv() (*flatbuffer.WriteUntimedCounterRequest, error) {
	return nil, nil
}
